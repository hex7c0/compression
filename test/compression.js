...
describe(
  ...
  function() {
    ...
    it('should skip HEAD', function(done) {
      ...
      request(server).head('/').set('Accept-Encoding', 'gzip').expect(
    ...
    it('should skip unknown accept-encoding', function(done) {
      ...
      request(server).get('/').set('Accept-Encoding', 'bogus').expect(
        shouldNotHaveHeader('Content-Encoding')).expect(200, done);
    });
    ...
    it('should skip if content-encoding already set', function(done) {
      ...
      request(server).get('/').set('Accept-Encoding', 'gzip').expect(
        'Content-Encoding', 'x-custom').expect(200, 'hello, world', done);
    });
    ...
    it('should set Vary', function(done) {
      ...
      request(server).get('/').set('Accept-Encoding', 'gzip').expect(
        'Content-Encoding', 'gzip').expect('Vary', 'Accept-Encoding', done);
    });
    ...
    it('should set Vary even if Accept-Encoding is not set', function(done) {
      ...
      request(server).get('/').expect('Vary', 'Accept-Encoding').expect(
        shouldNotHaveHeader('Content-Encoding')).expect(200, done);
    });
    ...
    it('should not set Vary if Content-Type does not pass filter',
      function(done) {
        ...
        request(server).get('/').expect(shouldNotHaveHeader('Vary')).expect(
          200, done);
      });
    ...
    it('should set Vary for HEAD request', function(done) {
      ...
      request(server).head('/').set('Accept-Encoding', 'gzip').expect('Vary',
        'Accept-Encoding', done);
    });
    ...
    it('should remove Content-Length for chunked', function(done) {
      ...
      request(server).get('/').expect('Content-Encoding', 'gzip').expect(
        shouldNotHaveHeader('Content-Length')).expect(200, done);
    });
    ...
    it('should work with encoding arguments', function(done) {
      ...
      request(server).get('/').set('Accept-Encoding', 'gzip').expect(
        'Transfer-Encoding', 'chunked').expect(200, 'hello, world', done);
    });
    ...
    it('should allow writing after close', function(done) {
      ...
      request(server).get('/').end(function() {
    ...
    it('should back-pressure when compressed', function(done) {
      ...
      request(server).get('/').request().on('response', function(res) {
    ...
    describe(
      ...
      function() {
        ...
        it('should not compress responses below the threshold size',
          function(done) {
        ...
        it('should compress responses above the threshold size',
          function(done) {
    ...
    describe('when "Accept-Encoding: deflate"', function() {
      ...
      it('should respond with deflate', function(done) {
        ...
        request(server).get('/').set('Accept-Encoding', 'deflate').expect(
    ...
    describe('res.flush()', function() {
      ...
      it('should flush the response', function(done) {
        ...
        request(server).get('/').set('Accept-Encoding', 'gzip').request().on(
      ...
      it('should flush small chunks for deflate', function(done) {
        ...
        request(server).get('/').set('Accept-Encoding', 'deflate').request()
...
function shouldHaveBodyLength(length) {
  ...
  return function(res) {
    ...
    assert.equal(res.text.length, length, 'should have body length of '
      + length);
  };
}
...
function shouldNotHaveHeader(header) {
  ...
  return function(res) {
    ...
    assert.ok(!(header.toLowerCase() in res.headers), 'should not have header '
      + header);
  };
}
